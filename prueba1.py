...
while True:
    ...
    if faces:
        ...
        if ratioAvg < 33 and startMicrosleep == 0:  # inicio de microsueño
            startMicrosleep = time.time()
        elif ratioAvg >= 33 and startMicrosleep != 0:  # fin de microsueño
            endMicrosleep = time.time()
            microsleepDuration = round(endMicrosleep - startMicrosleep, 2)
            if microsleepDuration >= 0.5:  # considera microsueño si dura al menos 0.5 segundos
                microsleepCounter += 1
            startMicrosleep = 0  # reinicia el tiempo de inicio
        ...
        cvzone.putTextRect(img, f'MicroSleep : {microsleepCounter}', (70, 160),
                           colorR=color)  # imprime el nro de microsueños por pantalla
