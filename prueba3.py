...
while True:
    ...
    if faces:
        ...
        ratio = int((lenghtVer / lenghtHor) * 100)  # impresion de valores longitud por terminal
        ...
        if ratioAvg < 33 and microsleepStart == 0:  # inicio de microsueño
            ...
            endMicrosleep = time.time()
            microsleepDuration = round(endMicrosleep - microsleepStart, 2)
            if microsleepDuration >= 0.5:  # considera microsueño si dura al menos 0.5 segundo
                microsleepCounter += 1
        ...
        if microsleepCounter == 2:
            alarma_audio.play()  # Reproduce el sonido de la alarma
            microsleepCounter = 0  # Reinicia el contador de microsueños a 0
            start_time = time.time()  # Tiempo de inicio para el temporizador
            while time.time() - start_time < 5:  # Muestra el mensaje durante 5 segundos
                cv2.putText(img, "Peligro: Reduciendo velocidad", (70, 220), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv2.imshow("Image", img)
                cv2.waitKey(25)
                if time.time() - start_time > 5:  # Detiene la reproducción del audio después de 5 segundos
                    alarma_audio.stop()
